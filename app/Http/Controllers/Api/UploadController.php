...
class UploadController extends Controller
{
    public function uploadpcount(Request $request)
    {
        ...
        $updated_store_code = false;
        if(count($filename_data) == 6){
            $updated_store_code = true;
            $store_code = explode(".", $filename_data[5]);
        ...
        if($updated_store_code){
            $store = Store::with('area')
                ...
                ->with('region')
                ->with('agency')
                ->where('store_code',$store_code[0])->first();
        }else{
            $store = Store::with('area')
                ->with('enrollment')
                ->with('distributor')
